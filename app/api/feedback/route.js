...
export async function POST(req) {
  try {
    ...
    // ガイドライン取得
    let guidelineFbQuery = supabase
      .from('guideline_items')
      .select('item_type, content, guideline_name, page_ref')
      .eq('disease_id', diseaseId)
    if (!shouldShowPreview()) {
      guidelineFbQuery = guidelineFbQuery.eq('is_active', true)
    }
    const { data: guidelines } = await guidelineFbQuery.limit(8)
...
2. 患者の性格・抵抗性に対して、介入数と説得の仕方が適切だったか（抵抗的な患者に対し、患者が同意していない介入を一方的に多数押し付けるとドロップアウトリスクを高める。ただし患者自身が同意・希望した生活指導はこの限りではない）
3. 患者が受け入れた治療の質（少ない介入でも高い同意率を得られた場合は高評価。患者が自ら多くの生活改善に同意した場合も同様に高評価とする）
   - **【最優先・絶対遵守】患者が自発的に示した意欲・同意に基づく生活指導の項目数は、多くても一切減点しない。「一度に多くの指導に同意させた」「欲張りすぎ」「患者の負担が大きい」など、同意・合意した項目数の多さ自体を批判材料・マイナス評価にしてはならない。患者が複数の生活改善に前向きであることはむしろ望ましい成果として積極的に評価する。実行可能性に懸念がある場合も、優先順位づけの助言として穏やかに述べるに留め、減点しないこと**
