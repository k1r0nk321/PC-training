...
export async function POST(request) {
  try {
    ...
    const formatRules = (rules) => rules.map(r =>
      `【${r.condition || '全般'}】${r.content}（エビデンスレベル${r.evidence_level || '不明'}、出典:${r.source || '不明'}）`
    ).join('\n')
    ...
    // 選択済み薬剤
    const selectedMedNames = (selectedMeds || []).map(m => m.drug_name_generic + '(' + m.drug_name_brand + ')').join('、') || 'なし'
    ...
    // 生活指導
    const selectedEduNames = (selectedEducation || []).map(e => e.label || e.instruction_key).join('、') || 'なし'
    const rejectedEduNames = (rejectedEducation || []).map(e => e.label || e.instruction_key).join('、') || 'なし'

    // コンサルト
    const consultText = (consultations || []).map(c => c.specialty + '(' + (c.reason || '') + ')').join('、') || 'なし'
