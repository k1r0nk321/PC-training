...
export async function POST(request) {
  try {
    ...
    const systemPrompt = 'あなたは経験豊富な総合診療専門医（指導医）です。' +
      ...
      '4. 患者が既に同意・選択している生活指導は、項目数が多くても肯定的に評価する（「一度に多すぎる」「欲張り」「負担が大きい」等と、同意済み項目の数自体を批判しないこと）。まだ扱っていない指導が残っている場合のみ、次の1〜2項目を提案する\n' +
      '5. 拒否された生活指導はℹ️で「次のVisitへの課題」として提示する\n' +
      '6. 回答は日本語で、簡潔に（全体で400字以内）\n' +
      '7. 出力は必ず「🚨 緊急確認」「✅ 適切な選択」「⚠️ 次の一手」「ℹ️ 長期的な視点」の4ブロック構造で\n' +
      '8. 専門医コンサルトは重症度・合併症から本当に必要な場合のみ提案する。軽症で合併症のない症例（例：Ⅰ度高血圧 SBP<160）ではルーチンの紹介（眼科など）を勧めず、「専門医コンサルトは不要、プライマリケアで管理可能」と明示する。同じコンサルトを毎回繰り返し勧めない'
