...
const DISEASE_CONFIGS = {
  ...
  '高尿酸血症・痛風': {
    ...
    labsHint: 'UA は必ず 7.0 mg/dL 以上、7.5-10.5 の範囲（高尿酸血症の主病態）。BMI が高いほど尿酸産生が増えるため、BMI 28以上では UA 9.0-10.5、BMI 25-27 では UA 8.0-9.5、BMI 25未満では UA 7.5-8.5 を目安に。UAクリアランス（ua_clearance）4.0-12.0 mL/min（排泄低下型は 7未満、産生過剰型は 10以上、正常範囲 7-12。BMI 高値・肥満例では産生過剰傾向が強く 9-12、痩せ型・高齢例では排泄低下型で 4-7 を考慮）。Cr 0.7-1.6、eGFR 40-95、Na 138-145、K 3.6-4.8、HbA1c 5.4-7.2、空腹時血糖 85-135、LDL 100-180、HDL 32-55、TG 120-300、AST 18-50、ALT 18-65（脂肪肝合併多い）、尿pH 5.0-6.5。',
...

{
  "patient": {
    ...
    "social_history": "生活歴（飲酒習慣・喫煙歴・運動習慣・食事習慣・外食頻度・夜食習慣を具体的に記載。下記の smoking_initial/drinking_initial と必ず整合させること）",
    "smoking_initial": "非喫煙者・喫煙継続・禁煙成功のいずれかからランダムに選択（減量中・禁煙挑戦中・再開はまれ。男性は喫煙継続が多め、女性は非喫煙者が多め）",
    "smoking_detail": "喫煙継続なら本数（例: 20本/日、10本/日）、禁煙成功なら禁煙年数（例: 5年前に禁煙、10年前に禁煙）、非喫煙者ならnull、減量中なら本数推移（例: 20本→10本/日）",
    "drinking_initial": "非飲酒者・適量・やや過量・過量のいずれかからランダムに選択（適量が最多）",
    "drinking_detail": "適量〜過量なら具体的な酒種と量（例: ビール 350mL/日、日本酒 1合/日、焼酎 2杯/日、ワイン 1-2杯/日）、非飲酒者ならnull。social_history と整合させること",
    "vitals": {
