...
export default function CaseDetailPage({ params }) {
  ...
  async function handleMedSelect(med) {
    ...
    const extraContext = newMedCount >= 2
      ? '今回で処方薬が' + newMedCount + '種類になる。薬剤数増加への反応も含める。'
      : null
    await addOrReplaceReaction('med_' + med.id, 'medication', med, '💊 ' + med.drug_name_generic + '（' + med.typical_dose + '）', extraContext)
  }
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '16px', alignItems: 'start' }}>
            <div>
              <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '16px', border: '1px solid #e2e8f0', marginBottom: '12px' }}>
                ...
                <p style={{ fontSize: '11px', color: '#64748b', marginBottom: '10px' }}>タグをクリック→指導内容を選択。▼のある項目は詳細な選択肢があります。</p>
                        ...
                        {items.map(function(item) {
                          ...
                          return (
                            <div key={item.id}
                              ...
                              style={{ padding: '5px 11px', borderRadius: '20px', fontSize: '12px', border: isSelected ? '2px solid #0369a1' : '1px solid #e2e8f0', backgroundColor: isSelected ? '#eff6ff' : 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                              ...
                              {subCount > 0 && <span style={{ fontSize: '10px', backgroundColor: '#0369a1', color: 'white', borderRadius: '8px', padding: '0 5px' }}>{subCount}</span>}
                            </div>
                          )
                        })}
                      </div>
        ...
        {activeEduModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', maxWidth: '520px', width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              <p style={{ fontSize: '13px', color: '#475569', marginBottom: '16px' }}>指導したい項目をカテゴリから選択してください。各カテゴリで1つ選択できます。</p>
        ...
        {activeSubGroupModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1100, padding: '16px' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', maxWidth: '480px', width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              <p style={{ fontSize: '12px', color: '#64748b', marginBottom: '12px' }}>1つ選択してください（選択ごとに右パネルの患者反応が更新されます）</p>
