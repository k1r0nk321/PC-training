...
## 【最重要・最新】2026-06-14 改訂サマリ（本節が以降のすべての記述に優先します）

> 対象 PR: #8（スマホ UI 改善）, #9（紹介状バグ修正）, #10（評価 AI：同意項目数）。すべて main にマージ・本番反映済み。

### 1. スマホ UI 改善（PR #8）

**患者情報カード（`PatientInfoCard`）を 2 セクションの独立折りたたみに分割**
- 旧：1 枚カードを単一トグル（`collapsed`/`onToggle`、初期は展開）で開閉。患者情報も検査データも一括表示で縦に間延び。
- 新：以下の 3 層構造に変更。
  1. 👤 **患者名ストリップ**（氏名・年齢・性別・疾患名）＝常時表示・トグルなし。
  2. 📋 **患者情報**（主訴・バイタル・既往/家族歴・生活歴。Visit2/3 は「本日/初診時バイタル＋前回の治療方針」）＝個別開閉・**初期は閉**。
  3. 💉 **検査データ**（検査結果。Visit2/3 は Visit タブ付き）＝個別開閉・**初期は閉**。検査が存在する場合のみ表示。
- 旧 props `collapsed`/`onToggle` と親 state `patientCardCollapsed` は**廃止**。コンポーネント内 `infoOpen`/`labsOpen`（ともに初期 `false`）で自己管理。`hasLabs` で検査セクションの表示要否を判定。
- 初診（`app/cases/[id]/page.js`）・Visit2・Visit3 の **3 ファイルに横展開**（各 `PatientInfoCard` 定義を個別に改修。第 11.11 節「完全コピー構造」に従う）。

**到達目標の「❓説明」ボタンを詳細モーダル内へ移動（`GoalsButton.js`）**
- 旧：ヘッダーに「🎯到達目標」と「❓説明」を横並び表示。
- 新：ヘッダーは「🎯到達目標」のみ。「❓説明」は到達目標の詳細モーダルのヘッダー内（タイトルと ✕ の間）へ移動。`helpOpen` の挙動自体は不変。

**問診の入力エリアを 2 行に圧縮（3 画面共通）**
- 「💬発言」ボタンを**削除**（Enter キーで送信できるため）。入力欄プレースホルダを「患者への質問を入力してEnterを押してください。」に変更。
- 「🔬診察・検査」と「治療方針を決定する →」を **2 カラムグリッドで横並び 1 列化**（旧：発言/検査の 2 列＋治療方針の全幅ボタンで計 2 行 → 新：1 行）。送信は `onKeyDown` の Enter ハンドラのみ。

### 2. 紹介状表示の配線バグ修正（PR #9）

- **不具合**：`referral-letter` API・画面の案内文・使い方ドキュメントは存在したが、問診の送信処理 `handleSend`（初診 `page.js`）から API を呼ぶ**配線が欠落**しており、「紹介状」と入力しても通常の患者 AI 応答になっていた。
- **修正**：`handleSend` 冒頭（user メッセージ追加直後）で、紹介状つき症例かつ入力に「紹介状」を含む場合に `/api/referral-letter`（`{ patientData }` → `{ letter }`）を呼び、結果を**システムメッセージ（`📄` プレフィックス）**として表示して `return`（患者 AI へは送らない）。
  - 紹介状つき症例の判定：`patient.chief_complaint + patient.history` に `紹介 / かかりつけ / 前医 / 閉院 / 転医 / 引き継ぎ` のいずれかを含む（既存の案内文ヒントと同条件）。
  - `📄` メッセージは患者 AI の **history から除外**（`/api/ai` 呼び出しの履歴フィルタに `📄` プレフィックス除外を追加）。
  - 紹介状ヒント・機能は**初診のみ**（Visit2/3 には無し。前医紹介状は初診持参の想定）。

### 3. 評価 AI：生活指導の同意項目数を減点しない（PR #10）

- **不具合**：患者が「塩分を控えたい」等の単純な意思を示すと複数の生活指導（減塩・DASH 食 等）への同意として扱われ、評価 AI が「一度に多くに同意させた（負担が大きい）」と批判していた。原因は評価プロンプトの「多介入はドロップアウトリスク」「少ない介入を高評価」という基準。
- **修正**：以下 3 つのプロンプトに「患者が自発的に同意した生活指導の**項目数の多さ自体を減点・批判しない**」ガードを統一挿入。
  - `app/api/feedback/route.js`（Visit1/2 フィードバック）：評価基準 2/3 を調整＋最優先ガードを追加。
  - `app/api/visit3-feedback/route.js`（最終評価）：「追加評価ポイント：生活指導の同意項目数」を追加。
  - `app/api/treatment-advice/route.js`（治療方針決定中の指導医アドバイス）：原則 4 を、同意済み項目数を批判せず肯定評価する内容に変更。
  - 方針：複数の生活改善に前向きなことは望ましい成果として評価。実行可能性の懸念は「優先順位づけの助言」に留め減点しない。

---

## 【最重要】2026-06-12 大規模改訂サマリ（以降、本節が旧記述に優先します）

### 1. 評価方式：点数制 → 到達目標の合否判定（全面変更）
- Visit 3 終了時に「到達目標 4 項目」を**すべて達成で合格**、1 つでも未達で**不合格**。100 点満点・各 Visit 33 点・4 軸採点は**廃止**。
- 到達目標 4 項目：
  1. 適切な生活指導（食事・運動。喫煙者は禁煙、過量飲酒者は節酒。患者の hidden_params で要否を自動判定）
  2. 適切な投薬の選択（AI が expected_medications と照合して判定。**学習者モードは対象外＝自動達成**）
  3. 適切な合併症の評価と管理（検査オーダー＋「症例が推奨する場合のみ」コンサルト依頼。**学習者モードは対象外**）
  4. 信頼度が Visit 3 終了時に 5 に到達
- 判定ロジック：`app/lib/achievement-goals.js`（`evaluateGoals`）。検査・コンサルト・生活指導は**全 Visit 累積**で判定（一度実施すれば達成を維持）。
- DB：`cases` に `passed`(boolean), `goal_breakdown`(jsonb) を追加。`final_score` は互換のため `passed?100:0` を保存。成績・結果画面は「合格/不合格」表示。
- ランキング称号は従来どおり経験症例数ベース（合格症例数＝final_score>=70 で集計）。

### 2. 患者特性依存の指導効果（visit2/visit3 route.js）
- 服薬指導：アドヒアランスが低い患者ほど効果大（low×1.8 / medium×1.0 / high×0.4、bonus 上限 +0.30）。
- 心理社会的支援（ストレス管理）：高ストレス患者で `effectiveAdherence` に +0.12。
- モニタリングは検査値への直接効果なし（現状）。

### 3. UI / 新コンポーネント
- 各 Visit ヘッダーに「🎯 到達目標」ボタン＋「❓説明」（`app/components/GoalsButton.js`）。達成度 N/4（学習者モードは N/2）をリアルタイム表示。
- ランクカード／バッジ（`RankCard.js` / `RankBadge.js`）＋研修医アバター（`public/avatars/trainee-p{1-4}-{male|female}.png`、フェーズ4段階×男女、性別は localStorage 保存）。トップ・各診療画面・成績一覧に表示。
- 症例選択：モデル症例は**簡単な順に1例ずつ解放**（合格で次が解放）。合格済みは別ボタンで復習選択。ランダム生成は別ボタン。モード選択タブは廃止。

### 4. コンサルト評価（consultation-evaluator.js）
- 高血圧の眼科コンサルトを**重症度条件付き（SBP≥160 のみ推奨）**に変更。軽症（Ⅰ度）では不要。
- `treatment-advice` も軽症・合併症なしではルーチン紹介（眼科等）を勧めない。

### 5. データ・ラベル変更（patient_education ほか）
- モデル症例タイトルを全疾患①〜⑤に統一・難易度昇順。difficulty は数値 1/2/3 に統一。
- 「体重管理（減量）」系の patient_education を**全疾患で削除**（食事・運動・投薬の結果のため単独項目にしない）。
- 「減塩指導（6g/日未満）」等のラベルを「減塩指導」に短縮。長文の `instruction_detail` は education API で短ラベルに正規化し、長文は `instruction_long` に退避。
- サブ選択肢カテゴリ `education` の表示見出しを「服薬指導の説明」→「概要・説明」に汎用化。
- サブグループが1つだけの指導項目はグループ一覧を飛ばして詳細選択へ直行。
...
> - 2026-06-14: スマホ UI 改善（PR #8：`PatientInfoCard` の 2 セクション個別折りたたみ化・初期閉、`GoalsButton` の「❓説明」をモーダル内へ移動、問診入力エリアの 2 行化）、紹介状表示の配線バグ修正（PR #9：`handleSend` から `/api/referral-letter` を呼ぶ処理を追加）、評価 AI が生活指導の同意項目数の多さを減点しないよう修正（PR #10：`feedback`/`visit3-feedback`/`treatment-advice`）。詳細は冒頭「2026-06-14 改訂サマリ」を参照。
